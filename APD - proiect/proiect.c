...
//variabila in care se va retine spectrul 
double *spectrum = 0;
							  ...
							  void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_TIMER_TICK:
			if(load==1)
			{
				GetCtrlVal(panel2, PANEL2_NUMERIC_NR_PUNCTE, &N);
				
				
				//plot semnal initial
				DeleteGraphPlot(panel2, PANEL2_GRAPH_RAW_SIGNAL, -1, VAL_IMMEDIATE_DRAW);
				PlotY(panel2, PANEL2_GRAPH_RAW_SIGNAL, waveData, N, VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, VAL_CONNECTED_POINTS, VAL_BLUE);
				
				//salvez imaginea initiala
				int imagineInitiala;
				char * file_name;
				file_name = (char *) calloc(100, sizeof(char));
				GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_RAW_SIGNAL, 1, &imagineInitiala);
				//in functie de filtru schimb numele imaginii si le salvez
				sprintf(file_name, "%s", "Images/Imagine initiala.jpg");
				SaveBitmapToPNGFile(imagineInitiala, file_name);
				DiscardBitmap(imagineInitiala);
			
				//generare fereastra
				int windowing;
				WindowConst constants;
	
 				GetCtrlVal (panel2, PANEL2_WINDOW,  &windowing);	
				...
				DeleteGraphPlot(panel2, PANEL2_GRAPH_WINDOW, -1, VAL_IMMEDIATE_DRAW);
   				PlotY (panel2, PANEL2_GRAPH_WINDOW, window, N, VAL_DOUBLE, VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_RED);
				
				// Se aplica o fereastra pentru a limita efectul sinc
				double sw[N];
				Mul1D (waveData, window, N, sw);
				DeleteGraphPlot(panel2, PANEL2_GRAPH_WINDOWING_RAW, -1, VAL_IMMEDIATE_DRAW);
				PlotY (panel2, PANEL2_GRAPH_WINDOWING_RAW, sw, N, VAL_DOUBLE, VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_RED);
	
			
				//spectru
				ScaledWindowEx (sw, N, RECTANGLE_, 0, &winConst);
				AutoPowerSpectrum(sw, N, dt, autoSpectrum, &df);
				PowerFrequencyEstimate (autoSpectrum, N/2, 0.0, winConst, df, 7, &frequencyPeak, &powerPeak);
				spectrum = (double*)calloc(N/2, sizeof(double));
				SpectrumUnitConversion(autoSpectrum, N/2, SPECTRUM_POWER, SCALING_MODE_LINEAR, DISPLAY_UNIT_VRMS, df, winConst, spectrum, unit);
				DeleteGraphPlot(panel2, PANEL2_GRAPH_SPECT_WIND_RAW, -1, VAL_IMMEDIATE_DRAW);
				PlotWaveform(panel2, PANEL2_GRAPH_SPECT_WIND_RAW, autoSpectrum, N, VAL_DOUBLE, 1.0, 0.0, 0.0, df, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, VAL_CONNECTED_POINTS, VAL_RED); //SA SCHIMB PLOT!!!!!!!
				...
				if(windowing==5)
				{
					int imagineSpectruInitial;
					char * file_name2;
					file_name2 = (char *) calloc(100, sizeof(char));
					GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_SPECT_WIND_RAW, 1, &imagineSpectruInitial);
					sprintf(file_name2, "%s", "Images/Imagine spectru pentru semnalul initial_Blackman.jpg");
					SaveBitmapToPNGFile(imagineSpectruInitial, file_name2);
					DiscardBitmap(imagineSpectruInitial);
				}
				else
				{
					int imagineSpectruInitial;
					...
					file_name2 = (char *) calloc(100, sizeof(char));
					GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_SPECT_WIND_RAW, 1, &imagineSpectruInitial);
					sprintf(file_name2, "%s", "Images/Imagine spectru pentru semnalul initial_FlatTop.jpg");
					SaveBitmapToPNGFile(imagineSpectruInitial, file_name2);
					DiscardBitmap(imagineSpectruInitial);
				}
				...
				//filtrari
				int filter;
				GetCtrlVal(panel2, PANEL2_SWITCH_FILTER, &filter);
				double coef[25];
				int n=25;
				double filtered[N+100];
				double fs = 1000.0; /* sampling frequency */
				double f1 = 200.0; /* the first stop band [0, 200] */
				double f2 = 250.0;
				double f3 = 350.0; /* the pass band [250, 350] */
				double f4 = 400.0; /* the second stop band [400, 500] */
				double delta=0.0;
				if(filter==1)
				{
					EquiRpl_BPF(sampleRate, 1./5*sampleRate, 1./4*sampleRate, 35./100*sampleRate, 4./10*sampleRate, n, coef, &delta);
					Convolve(coef, n, waveData, N, filtered);
					DeleteGraphPlot(panel2, PANEL2_GRAPH_FILT, -1, VAL_IMMEDIATE_DRAW);
					PlotY(panel2, PANEL2_GRAPH_FILT, filtered, N, VAL_DOUBLE, VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GREEN);
					
					//salvez imaginea pentru semnalul filtrat
					int imagine_filtrata;
					char * file_name3;
					file_name3 = (char *) calloc(100, sizeof(char));
					GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_FILT, 1, &imagine_filtrata);
					sprintf(file_name3, "%s", "Images/EquiRpl_BPF.jpg");
					SaveBitmapToPNGFile(imagine_filtrata, file_name3);
					DiscardBitmap(imagine_filtrata);
				...
				{
					int order=5;
					Bssl_BPF(waveData, N, sampleRate, sampleRate/4, sampleRate/2, 5, filtered);
					DeleteGraphPlot(panel2, PANEL2_GRAPH_FILT, -1, VAL_IMMEDIATE_DRAW);
					PlotY(panel2, PANEL2_GRAPH_FILT, filtered, N, VAL_DOUBLE, VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_RED);
					...
					file_name3 = (char *) calloc(100, sizeof(char));
					GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_FILT, 1, &imagine_filtrata);
					sprintf(file_name3, "%s", "Images/Bessel.jpg");
					SaveBitmapToPNGFile(imagine_filtrata, file_name3);
					DiscardBitmap(imagine_filtrata);
				}
				...
				// Se aplica o fereastra pentru a limita efectul sinc
				double sw2[N];
				Mul1D (filtered, window, N, sw2);
				DeleteGraphPlot(panel2, PANEL2_GRAPH_WINDOWING_FILT, -1, VAL_IMMEDIATE_DRAW);
				PlotY (panel2, PANEL2_GRAPH_WINDOWING_FILT, sw2, N, VAL_DOUBLE, VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_RED);
				...
				double autoSpectrum2[npoints];
				//double dt1=1.0/sampleRate;
				double df2=0.0;
				double dt2=1.0/sampleRate;
				double frequencyPeak2;
				double powerPeak2;
				char unit2[32] = "V";
				ScaledWindowEx (sw2, N, RECTANGLE_, 0, &winConst2);
				AutoPowerSpectrum(sw2, N, dt, autoSpectrum2, &df2);
				PowerFrequencyEstimate (autoSpectrum2, N/2, 0.0, winConst2, df2, 7, &frequencyPeak2, &powerPeak2);
				double *spectrum2 = (double*)calloc(N/2, sizeof(double));
				SpectrumUnitConversion(autoSpectrum2, N/2, SPECTRUM_POWER, SCALING_MODE_LINEAR, DISPLAY_UNIT_VRMS, df2, winConst2, spectrum2, unit2);
				DeleteGraphPlot(panel, PANEL2_GRAPH_SPECT_WIND_FILT,-1,VAL_IMMEDIATE_DRAW);
				PlotWaveform(panel2, PANEL2_GRAPH_SPECT_WIND_FILT, autoSpectrum2, N, VAL_DOUBLE, 1.0, 0.0, 0.0, df2, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, VAL_CONNECTED_POINTS, VAL_RED); 
				
				
				//salvez imaginea pentru semnalul filtrat si ferestruit
				if(windowing==5)
				{
					if(filter==1)
					{
						int imagine_fil_fer;
						char * file_name4;
						file_name4 = (char *) calloc(100, sizeof(char));
						GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_WINDOWING_FILT, 1, &imagine_fil_fer);
						sprintf(file_name4, "%s", "Images/EquiRpl_BPF_Blackman.jpg");
						SaveBitmapToPNGFile(imagine_fil_fer, file_name4);
						DiscardBitmap(imagine_fil_fer);
					}
					else
					{
						int imagine_fil_fer;
						...
						file_name4 = (char *) calloc(100, sizeof(char));
						GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_WINDOWING_FILT, 1, &imagine_fil_fer);
						sprintf(file_name4, "%s", "Images/Bessel_Blackman.jpg");
						SaveBitmapToPNGFile(imagine_fil_fer, file_name4);
						DiscardBitmap(imagine_fil_fer);
					}
				...
				else
				{
					if(filter==1)
					{
						...
						file_name4 = (char *) calloc(100, sizeof(char));
						GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_WINDOWING_FILT, 1, &imagine_fil_fer);
						sprintf(file_name4, "%s", "Images/EquiRpl_BPF_FlatTop.jpg");
						SaveBitmapToPNGFile(imagine_fil_fer, file_name4);
						DiscardBitmap(imagine_fil_fer);
					}
					else
					{
						...
						file_name4 = (char *) calloc(100, sizeof(char));
						GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_WINDOWING_FILT, 1, &imagine_fil_fer);
						sprintf(file_name4, "%s", "Images/Bessel_FlatTop.jpg");
						SaveBitmapToPNGFile(imagine_fil_fer, file_name4);
						DiscardBitmap(imagine_fil_fer);
					}
					...
					//salvez imaginea pentru spectrul pe semnalul filtrat si ferestruit
				if(windowing==5)
				{
					if(filter==1)
					{
						...
						char * file_name4;
						file_name4 = (char *) calloc(100, sizeof(char));
						GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_SPECT_WIND_FILT, 1, &imagine_fil_fer);
						sprintf(file_name4, "%s", "Images/Spectru_EquiRpl_BPF_Blackman.jpg");
						SaveBitmapToPNGFile(imagine_fil_fer, file_name4);
						DiscardBitmap(imagine_fil_fer);
					}
					else
					{
						...
						file_name4 = (char *) calloc(100, sizeof(char));
						GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_SPECT_WIND_FILT, 1, &imagine_fil_fer);
						sprintf(file_name4, "%s", "Images/Spectru_Bessel_Blackman.jpg");
						SaveBitmapToPNGFile(imagine_fil_fer, file_name4);
						DiscardBitmap(imagine_fil_fer);
					}
				...
				else
				{
					if(filter==1)
					{
						...
						file_name4 = (char *) calloc(100, sizeof(char));
						GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_SPECT_WIND_FILT, 1, &imagine_fil_fer);
						sprintf(file_name4, "%s", "Images/Spectr_EquiRpl_BPF_FlatTop.jpg");
						SaveBitmapToPNGFile(imagine_fil_fer, file_name4);
						DiscardBitmap(imagine_fil_fer);
					}
					else
					{
						...
						file_name4 = (char *) calloc(100, sizeof(char));
						GetCtrlDisplayBitmap(panel2, PANEL2_GRAPH_SPECT_WIND_FILT, 1, &imagine_fil_fer);
						sprintf(file_name4, "%s", "Images/Spectru_Bessel_FlatTop.jpg");
						SaveBitmapToPNGFile(imagine_fil_fer, file_name4);
						DiscardBitmap(imagine_fil_fer);
					}
